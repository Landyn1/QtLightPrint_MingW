...
QPainterPath MyGraphicsPolygonItem::ViewPath(int k)
{
    QPainterPath p;
...
void MyGraphicsPolygonItem::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
   ...
   //pen.setColor(QColor(0, 160, 230));
    ...
    if(data(15).toInt() == 1)
    {
        ...
        painter->scale(1,-1);
        painter->drawPixmap(QRectF(p1.x()-w,p1.y()-h-40/s,20/s,20/s),pix,pix.rect());
        painter->scale(1,-1);
    }
...
bool MyGraphicsPolygonItem::set_brush(double angle,double space)
{
    brushpath.clear();
    this->angle = angle;
...
void MyGraphicsPolygonItem::rotateY()
{
...
void MyGraphicsPolygonItem::rotateX()
{
...
QRectF MyGraphicsPolygonItem::getRect()
{
    double x,y,w,h;
